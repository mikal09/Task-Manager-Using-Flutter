...
class DBHelper {
  ...
  _onCreate(Database db, int version) async {
    await db.execute(
        "CREATE TABLE $TABLE ($ID INTEGER PRIMARY KEY, $NAME TEXT, $ISTRUE INTEGER)");
  }

  Future<int> save(Employee employee) async {
    print(employee.isTrue);
  ...
  Future<List<Employee>> getEmployees() async {
    var dbClient = await db;
    List<Map> maps = await dbClient.query(TABLE, columns: [ID, NAME, ISTRUE]);
